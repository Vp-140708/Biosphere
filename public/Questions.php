<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Вопрос-Ответ - Ветеринарная Клиника</title>
    <link rel="stylesheet" href="CSS/style_q.css" />
    <link rel="stylesheet" href="CSS\styles1.css" />
  </head>
  <body>
    <header class="header">
      <div class="header-container">
        <div class="left-section1">
          <a href="index.html" class="logo-link">
            <img
              src="assets/images/bio-logo1.png"
              alt="Site Logo"
              class="logo"
            />
            <span class="site-name">Биосфера</span></a
          >
          <button type="button" class="theme-switcher" id="theme-switcher">
            🌞
          </button>
        </div>
        <nav class="nav-links">
          <a href="Register.html">Регистрация</a>
          <a href="Questions.html">Вопрос-Ответ</a>
          <a href="Price list.html">Прейскурант</a>
          <a href="Staff1.html">Специалисты</a>
          <a href="Филиалы1.html">Филиалы</a>
          <a href="Отзывы1.html">Отзывы</a>
          <a href="https://vk.com/biosre" target="_blank"
            ><img
              src="../assets/images/media_social_vk_vkontakte_icon_124252 (1).png"
              alt="Вк"
          /></a>
        </nav>
        <div class="hamburger-menu">
          <div class="hamburger"></div>
        </div>
        <div class="dropdown">
          <a href="Register.html">Регистрация</a>
          <a href="Questions.html">Вопрос-Ответ</a>
          <a href="Price list.html">Прейскурант</a>
          <a href="Staff1.html">Специалисты</a>
          <a href="Филиалы1.html">Филиалы</a>
          <a href="Отзывы1.html">Отзывы</a>
          <a href="https://vk.com/biosre" target="_blank"
            ><img
              src="../assets/images/media_social_vk_vkontakte_icon_124252 (lit).png"
              alt="ВК"
          /></a>
        </div>
      </div>
    </header>
    <div class="container">
      <h2>Оставьте свой вопрос</h2>
      <div id="questionForm" class="form-control">
        <input
          type="text"
          id="username"
          placeholder="Введите ваше имя"
          required
        />
        <input
          type="text"
          id="phone"
          placeholder="Введите ваш номер телефона (по желанию)"
        />
        <input
          type="text"
          id="email"
          placeholder="Введите ваш email(по желанию)"
        />
        <textarea id="question" placeholder="Введите ваш вопрос"></textarea>
        <button type="submit">Отправить</button>
      </div>
    </div>
    <div class="container">
      <h2>Заданные вопросы</h2>
      <div
        id="questionsContainer"
        class="form-control"
        style="width: 100%; margin: auto auto"
      ></div>
    </div>
    <button id="logout-btn" style="position: fixed; right: 10px; bottom: 20px; width: 15em; z-index: 100;">Выйти из аккаунта</button>
    <script src="JS/script_q.js"></script>
    <script src="JS/script1.js"></script>
  </body>
</html>
